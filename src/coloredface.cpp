...
void ColoredFace::startTrial()
{
    ...
    else if (state == trial_state::POST_STIMULUS){
        pauseFlashing();
...
void ColoredFace::startFlashing()
{
    ...
    sendMarker(config::OVTK_StimulationLabel_Base + flashingSequence->sequence[currentStimulation]);

    // send target marker
    if (spelling_mode == operation_mode::CALIBRATION || spelling_mode == operation_mode::COPY_MODE)
    {
        if (isTarget())
        {
    ...
    if(speller_type == speller_type::COLORED_FACE)
    {
        ...
        {
            this->layout()->itemAt(flashingSequence->sequence[currentStimulation])->
                    widget()->setStyleSheet("image: url(:/images/assets/bennabi_face_brown.png)");
            break;
        }
        case 5:
        {
    ...
    else if(speller_type == speller_type::INVERTED_COLORED_FACE)
    {
        switch (flashingSequence->sequence[currentStimulation])
        {
        case 1:
        ...
        {
            this->layout()->itemAt(flashingSequence->sequence[currentStimulation])->
                    widget()->setStyleSheet("image: url(:/images/assets/assets/bennabi_face_green_inverted.png)");
            break;
        }
        case 3:
        {
        ...
        {
            this->layout()->itemAt(flashingSequence->sequence[currentStimulation])->
                    widget()->setStyleSheet("image: url(:/images/assets/bennabi_face_brown_inverted.png)");
            break;
        }
        case 5:
        {
    ...
    isiTimer->stop();
    state = trial_state::POST_STIMULUS;
}
...
void ColoredFace::pauseFlashing()
{
    ...
    if (currentStimulation >= flashingSequence->sequence.count())
    {
        ...
        state = trial_state::FEEDBACK;

        if(spelling_mode == operation_mode::COPY_MODE || spelling_mode == operation_mode::FREE_MODE){
            feedback();
        }
        else if(spelling_mode == operation_mode::CALIBRATION)
        {
            post_trial();
        }
    }
...
void ColoredFace::pre_trial()
{
    ...
    if (pre_trial_count == 0)
    {
        ...
        if (spelling_mode == operation_mode::CALIBRATION)
        {
        ...
        else if(spelling_mode == operation_mode::COPY_MODE)
        {
            highlightTarget();
        }
    }
...
void ColoredFace::feedback()
{
    ...
    if (spelling_mode == operation_mode::COPY_MODE)
    {
...
void ColoredFace::post_trial()
{
    ...
    if (currentLetter >= desired_phrase.length() && (spelling_mode == operation_mode::COPY_MODE || spelling_mode == operation_mode::CALIBRATION)){
        qDebug()<< "Experiment End";
