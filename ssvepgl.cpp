...
//        this->setWidth( QGuiApplication::screens().last()->width() );
//        this->setHeight( QGuiApplication::screens().last()->height() );
        this->showFullScreen();
    }
    ...
    {
        flicker[i] = gen_flick(frequencies[i], REFRESH_RATE, stimulationDuration);
    }
